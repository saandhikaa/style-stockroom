...
@section('main')
    ...
    <form action="/dashboard/manage-products/{{ $product->slug }}" method="post" class="w-full mt-5" enctype="multipart/form-data">
        @method('put')
        @csrf
        ...
        <div class="items-center mb-6">
            ...
            <img src="{{ asset('images/' . $product->image) }}" class="img-prewiew w-40 mb-2 rounded">
            <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500 {{ $errors->has('image') ? 'border-red-500' : '' }}" id="image" name="image" type="file" onchange="previewImage()">
            @error('image')
                <span class="text-red-500 text-xs italic">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="items-center mb-6">
            ...
            <div class="sizes-container flex flex-wrap items-center">
                @foreach(old('sizes', $product->sizesArray, ['']) as $size)
